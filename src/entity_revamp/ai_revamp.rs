...
pub type AIfn = fn(ctx: &mut Context, entity: &mut Entity, player: &mut Entity, enemies: &mut VecDeque<Entity>, walls: &mut Vec<Entity>);
...
impl AI {
    ...
    pub fn execute(&self, ctx: &mut Context, entity: &mut Entity, player: &mut Entity, enemies: &mut VecDeque<Entity>, walls: &mut Vec<Entity>) {
        for ai in &self.calls {
            ai(ctx, entity, player, enemies, walls);
        }
    }
}
...
pub fn chase_player(ctx: &mut Context, entity: &mut Entity, player: &mut Entity, enemies: &mut VecDeque<Entity>, walls: &mut Vec<Entity>) {
    ...
    for e in iter::once(&mut player.clone()).chain(enemies.iter_mut()).chain(walls.iter_mut()) {
...
pub fn player_input(ctx: &mut Context, player: &mut Entity, _ignore: &mut Entity, enemies: &mut VecDeque<Entity>, walls: &mut Vec<Entity>) {
    ...
    // undo move if collision occurred
    for e in enemies.iter().chain(walls.iter()) {
